...
<style>
  ...
  .it-stats-mini-value {
    margin: 0.24rem 0 0;
    font: 780 1.08rem/1.15 "Space Grotesk", sans-serif;
    color: var(--text-1);
  }
...
<script>
  (() => {
    ...
    const statsMonthValue = document.querySelector("#it-stats-month-value");
    const statsMonthNote = document.querySelector("#it-stats-month-note");
    ...
    const loadStatsRecap = async () => {
      try {
        const response = await fetch("/api/italy-statistics.php", { headers: { Accept: "application/json" } });
        if (!response.ok) throw new Error("Request failed");
        const payload = await response.json();
        if (!payload || payload.ok !== true) throw new Error("Payload invalid");
